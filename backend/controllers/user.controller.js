...
const registerUser = async (req, res) => {
  try {
    ...
    //  AUTO-ASSIGN EXISTING ACTIVE KPIs TO NEW USER
    try {
      await assignExistingKPIsToUser(user)
    } catch (kpiError) {
      console.error('KPI Assignment Error (non-blocking):', kpiError)
      // Don't fail registration if KPI assignment fails
    }
...
/**
 * Assign existing active KPIs to a new user based on their role and battalion
 */
const assignExistingKPIsToUser = async (user) => {
  try {
    // Find all active KPIs that should be assigned to this user
    const eligibleKPIs = await KPI.find({
      $or: [
        // KPIs assigned to all users
        { 'targets.allUsers': true },
        // KPIs assigned to user's role
        { 'targets.roles': { $in: [user.role] } },
        // KPIs assigned to user's battalion
        { 'targets.battalions': { $in: [user.battalion] } },
        // KPIs specifically assigned to this user (if any)
        { 'targets.specificUsers': { $in: [user._id] } }
      ],
      // Only active KPIs (not deleted and deadline not passed)
      deadline: { $gte: new Date() }
    })

    if (eligibleKPIs.length === 0) {
      console.log(`No eligible KPIs found for new user ${user.email}`)
      return
    }

    // Create KPIStatus entries for each eligible KPI
    const statusEntries = eligibleKPIs.map(kpi => ({
      user: user._id,
      kpi: kpi._id,
      status: 'pending',
      progress: 0,
    ...
    console.log(`✅ Assigned ${statusEntries.length} existing KPIs to new user ${user.email}`)

    // Update stats for affected KPIs
    const kpiIds = eligibleKPIs.map(kpi => kpi._id)
    for (const kpiId of kpiIds) {
      try {
        ...
        console.error(`Error updating stats for KPI ${kpiId}:`, statsError)
      }
    }
  } catch (error) {
    console.error('Error assigning existing KPIs to user:', error)
    throw error
  }
}
    ...
    const myKpis = userKpiStatuses.map(status => {
      ...
      return {
        ...
        kpi: {
          ...
          assignmentType: kpi.targets?.allUsers
            ? 'all'
            : kpi.targets?.roles?.length > 0
              ? 'role'
              : kpi.targets?.battalions?.length > 0 ? 'battalion' : 'specific',
          target: kpi.targets?.allUsers
            ? 'All Users'
            : kpi.targets?.roles?.length > 0
              ? kpi.targets.roles.join(', ')
...
const updateUserStatus = async (req, res) => {
  try {
    ...
    // Send account status change notification if status actually changed
    if (oldStatus !== status) {
      const reason = status === 'active'
        ? 'Account reactivated by administrator'
        : 'Account deactivated by administrator'

      await notificationService.sendAccountStatusChangeNotification(
        userId,
        status,
        reason
      )
